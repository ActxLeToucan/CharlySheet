    ...
    <div class="flex justify-end h-0 w-full pointer-events-none">
        ...
        >
            <div
                v-for="el in m_data"
                :key="el.id"
                class="flex cursor-pointer hover:bg-slate-50 hover:dark:bg-slate-500 px-2 py-1 min-w-full rounded-md border-2 border-transparent
                       w-full space-x-2 hover:border-slate-200 hover:dark:border-slate-400 hover:w-fit"
                @click="() => {onclicked(el);}"
            >
                <div
                    class="flex rounded-full bg-white dark:bg-slate-600 border-2 w-8 h-8 items-center justify-center shadow-md transition-all overflow-hidden my-auto"
                    :style="`border-color: ${el.color};`"
                >
                    <p class="text-slate-600 dark:text-slate-50 font-extrabold">
